...
func terminalOriginAllowed(r *http.Request) bool {
	...
	// Fetch Metadata is generated by the browser and survives proxies that
	// rewrite Host without forwarding the original host or scheme.
	...
	if err != nil || originURL.Host == "" || (originURL.Scheme != "http" && originURL.Scheme != "https") {
		log.Printf("Rejected terminal WebSocket with invalid origin %q", origin)
		return false
	}
	...
	allowed := strings.EqualFold(originURL.Scheme, scheme) &&
		canonicalOriginHost(originURL.Host, originURL.Scheme) == canonicalOriginHost(host, scheme)
	if !allowed {
		log.Printf("Rejected terminal WebSocket origin=%q host=%q forwardedHost=%q forwardedProto=%q fetchSite=%q",
			origin, r.Host, r.Header.Get("X-Forwarded-Host"), r.Header.Get("X-Forwarded-Proto"), r.Header.Get("Sec-Fetch-Site"))
	}
	return allowed
}
